...
def get_historical_hourly_price(
    ...
    data_frame['Weighted_Price'] = data_frame[["high", "low"]].mean(axis=1)
    data_frame['Weighted_Price'] = data_frame['Weighted_Price'].astype('float64')
...
if __name__ == '__main__':
    ...
    file_name = str(args.s1 + '-' + args.s2 + '_data.csv')
    market_data = get_historical_hourly_price(args.s1, args.s2, hours, TIME_DELTA)

    # store = pd.HDFStore('data/market_data/' + file_name)
    market_data.to_csv(r"C:\Users\Musta\PycharmProjects\Bachelorarbeit\data\market_data\{0}".format(file_name))
